...
**This module is ``acquire.driver``.** Patch 4 did not leave the flat module behind as a
re-export shell: ``acquire/driver.py`` publishes *this* module under its own name
(``sys.modules['udv_echo_process.acquire.driver'] is session``), because a copy of a name is not
the name a patch rebinds — ``driver`` has to be the module whose own globals the bodies above read,
or every fake that scripts ``driver._gui`` would silently stop reaching them. Every name the flat
module published at Patch 3's tip still resolves from ``driver``, and the live-proven gestures are
**byte-identical** to the ones the live sessions proved: this refactor moves code and corrects
interpretation rules, never a workflow.
...
# resolving too. ``COMBO_ORDER`` below is this module's own: the resolve reads it.
