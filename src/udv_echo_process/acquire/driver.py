...
``acquire/udop/recording.py``'s, the store surface — the Store dialog, its name, working
directory, accept and stored file — is ``acquire/udop/store.py``'s, and the ``Parameters``
surface — the popup, the dialog and the channel — is ``acquire/udop/parameters.py``'s, moved verbatim and composed back into this
class as :class:`ParametersSurface`. The moved bodies reach their collaborators through ``self``, so they
...
# The four workflow surfaces Patch 4 splits out of this class. Three of them are composed into
# :class:`Win32Actuator` below; the names beside them are re-exported for the callers and the tests
# that were written against the flat module — the surfaces' own timing knobs and the ``Parameters``
# module's two helpers — so ``driver.GESTURE_POSTED_PRESS``, ``driver.DIALOG_FILL_TIMEOUT_S``,
# ``driver._POLL_S``, ``driver.channel_items`` and the rest keep resolving. ``AcquisitionError`` is
# the ``udop`` package's one failure class now (``acquire/udop/__init__.py``, so no surface has to
# import a sibling to raise it), so it is imported here rather than defined below.
...
# ``DIALOG_FILL_TIMEOUT_S`` (how long the dialog is given to state its values),
# ``_ENTRY_DIALOG_TIMEOUT_S`` (how long one popup entry's press is given to produce a dialog),
# ``_DIALOG_REPLACE_S`` (how long the application is given to replace the dialog after a channel
# write), ``_MENU_TIMEOUT_S``/``_MENU_POLL_S`` (the popup's own wait and its cadence) and
# ``GESTURE_POSTED_PRESS`` (the gesture a popup entry is taken with) were defined here. They are
# ``acquire/udop/parameters.py``'s as of Patch 4, moved there with the loops that read them — a
# constant lives with the code that reads it, which is the rule Patch 3 followed when it left them
# in this module — and are imported at the top of this file under these same names, so
# ``driver.DIALOG_FILL_TIMEOUT_S``, ``driver._MENU_POLL_S``, ``driver.GESTURE_POSTED_PRESS`` and
# the rest keep resolving. Their own docstrings travelled with them.
...
# ``channel_items`` and ``_descendants`` were defined here: the channel list the dialog's
# combo is identified by, and the breadth-first walk over an already-resolved role map. They
# are ``acquire/udop/parameters.py``'s as of Patch 4, moved with their only caller
# (``_channel_combo``), and are imported at the top of this file under these same names, so
# ``driver.channel_items`` and ``driver._descendants`` keep resolving.
