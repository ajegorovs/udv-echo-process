...
| the profile-global `windows-gui-automation` skill | **was at its size limit** (100,831 characters against Hermes's 100,000 cap, so every patch to it was refused). Split on 2026-09-18 into a 33 KB router plus `references/pitfalls.md` and `references/evidence-discipline.md`, verbatim and with a zero-loss check — it is patchable again. It still carries instrument-specific material that belongs in the repo overlay (`.agents/skills/udop-acquisition/`), which is the reconciliation left undone |
