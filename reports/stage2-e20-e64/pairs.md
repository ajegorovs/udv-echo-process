...
This Stage-2 analysis does not rewrite `reports/sparse-mixer-live-1/decision-table.md`: its E64-vs-E20 row originally read `defer` / not resolvable **with that pass's design**, and this campaign was its named overturning measurement. A separate reviewed decision slice has since updated that row to `replace` / not detected at this campaign's floor; the measurement and the later decision remain separate artefacts.
