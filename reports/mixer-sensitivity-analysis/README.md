...
| `reference-repeat.csv` | WP1 | same-settings repeatability/drift screening threshold versus depth |
| `figures/reference-repeat.*` | WP1 | reviewer-visible repeat comparison |
| `resolution-levels.csv` | WP2 | one row per decoded pitch — 13 levels selected from 14 recordings — on its own native gate grid |
| `resolution-pairs.csv` | WP2 | every unordered pair of the 13 levels on common knots, against the WP1 screening threshold |
...
| `burst-levels.csv` | WP2 | one row per decoded burst length — 13 levels selected from 14 recordings — native-grid and temporal metrics |
| `burst-pairs.csv` | WP2 | every unordered pair of the 13 levels on the shared knots, against the WP1 screening threshold |
...
| `prf-levels.csv` | WP2 | one row per decoded PRF period — 5 levels selected from 6 recordings: velocity load, wrap-like counts, matched-segment temporal metrics |
| `prf-pairs.csv` | WP2 | every unordered pair of the 5 levels on the shared knots and bands, against the WP1 screening threshold |
...
| `gain-power-levels.csv` | WP2 | one row per screened TGC/emitting-power level — 9 TGC levels from 10 recordings, 3 power levels from 4 recordings: key, common window and support, dropout/spread screening |
| `gain-power-pairs.csv` | WP2 | every within-axis pair (36 TGC, 3 power) on common knots, against the WP1 screening threshold, with the depth ranges where it clears |
...
| `figures/gain-power-screen.*` | WP2 | reviewer-visible dropout-by-depth and focus-pair-against-threshold decision figure |
...
Every generated artifact above is pinned by the SHA-256 of its **committed bytes**, at the corrected revision
each provenance document records. `decision-table.md` carries the same list beside the cells it reads, and
`tools/validate_analysis_review_baseline.py --check-final` re-checks this one, so a stale row here is a gate
failure rather than a note. Paths are relative to this directory; the repository-root `.gitattributes` pins
`text eol=lf` on `*.csv`/`*.json` and `binary` on `figures/*.png`, so the numbers survive a Windows checkout.

| Artifact | SHA-256 |
|---|---|
| `manifest.csv` | `064a289b7b9e8128766305ac047ca171355539d83461fc25a25dcec1cf4d7139` |
| `qc-summary.json` | `4e46582b6cfe1644cc3699a5134d8c02dcf9ad3e7568359a01a2f1c56fce463f` |
| `reference-repeat.csv` | `05f3495c98f6d58fd50cb447fbca4c8a1164aba178c6b53d0eefa34a58c4d9cb` |
| `reference-repeat.provenance.json` | `b0c1b7cf9639f83fd63d8fb2c39414490bba98a1e7f846de94b8608527a0061b` |
...
| `burst-ladder.provenance.json` | `f0e70fe4c70a953b39c61878ee4635b17d376a619cfbc6171441f6f76fb5612c` |
| `figures/burst-ladder.png` | `0fb98cebeb0f19ac924ab5aa81638be740b308ec7660796b20cbe04435208afa` |
| `prf-levels.csv` | `adf8320336f6de4265df9c666c7a12515f22e1d552e50d07cbd48d69843fa997` |
| `prf-pairs.csv` | `b1e9399e7a96213dcec3abac585a7174bd65fc08815f641c3f4d19c5bf1220c3` |
| `prf-ladder.provenance.json` | `da503728dee8d6c6a3de176c4d72d4b316b938be3b96f617e7ff8cf250ef94a3` |
| `figures/prf-ladder.png` | `dccccebe47666f25abd6002df5a793862bea0b16134c79aa339283346b687a23` |
| `gain-power-levels.csv` | `a1417023cee709d8a050a5d7fb90996bd660c4e4b90ef2eb800ddddb9c2df195` |
| `gain-power-pairs.csv` | `ee2fb3e1d0332c0b72110513df69b77a7d73563e2a0d3395da6ef20c46e19dbc` |
| `gain-power-depths.csv` | `d5e539420610e71c59272376bdcb30a27815bed0f11d43b823b4661b358bb335` |
| `gain-power-screen.provenance.json` | `2597de32623a58f6af3af3f9f25ca0c0c6d5ac631212f9d129d83c683a453301` |
| `figures/gain-power-screen.png` | `eb5fd8a09a482841c7f037c038cfbdc8a05cb980362db282d67282fc92b21cd5` |
...
## WP1 — the sole-pair observed-discrepancy screening threshold
...
is the **sole-pair observed-discrepancy screening threshold**, because duration and unknown acquisition time also
differ — the files carry no acquisition order, so drift cannot be bounded or reconstructed; it is
screened, not bounded. The
WP1 artefacts are:
...
| `reference-repeat.provenance.json` | the binding (manifest hash, both source hashes, generator commit), the metric definitions, both time views, the per-file timestamp statistics (median Δt, Δt IQR, RMS deviation and maximum deviation) with the estimator decision, the temporal parameters with the full ACF/PSD curves, the declared screening threshold and the figure caption |
| `figures/reference-repeat.png` | four panels: the two depth profiles, the signed difference against the declared screening threshold, the ensemble autocorrelation and the ensemble PSD on the identical 50-gate grid |
...
- **sole-pair observed-discrepancy screening threshold** — the largest absolute per-gate mean difference over depth, with the gate
  and depth where it occurs. It is a screening threshold: one observed realization of
  repeatability *plus* uncontrolled drift, not a
...
- **timestamp statistics** — the temporal view's uniform grid is a *stated* method, not a substitute for
  the recorded timestamps: the provenance publishes each file's median Δt, Δt IQR, RMS deviation and
  maximum deviation beside the quantitative criterion and the estimator decision (R6).
...
The corrected ladder is a setting-based selection of **13 decoded gate pitches from 14 recordings** over
the same window — 0.247 mm (`res/0-2.BDD`, 365 gates) to 2.96 mm (`res/3-0.BDD`, 31 gates) — and it carries
the plan's first question: *does 0.247 mm add information over 0.617 mm, and which measured pitch is the
coarsest whose aligned difference remains small relative to the screening threshold?* Selection is by the
decoded fingerprint, not by folder: every recording whose decoded settings put it at a pitch of this ladder
is a realization of that level, so 12 of the 13 levels are a single recording and the shared 1.850 mm
reference level carries both reference recordings (`prf/600.BDD`, `res/1-8.BDD`) as two named realizations
(`findings.realizations`). **One duplicated setting is not replicated coverage of the axis**: the reference
level is one condition measured twice, every other level is one recording, and each level's numbers are the
unweighted mean of its realizations' numbers. The resolution artefacts are:

| Path | What it is |
|---|---|
| `resolution-levels.csv` | 13 rows, one per decoded pitch (selected from 14 recordings, with the shared 1.850 mm reference level carrying two named realizations): common-duration mean, robust spread (IQR), RMS, zero fraction and gate-level temporal IQR inside the common support, plus the native-grid gradient spread and a descriptive autocorrelation scale of one mean-removed depth profile |
| `resolution-pairs.csv` | 78 rows, every unordered pair of the 13 levels: the signed `fine - coarse` per-gate mean difference on the coarser grid's own knots, the knots that clear the WP1 screening threshold and the depth ranges where they do, and the finer profile's normalized reconstruction-residual variance below the coarse knot spacing |
| `resolution-ladder.provenance.json` | the binding (manifest hash, all 14 source hashes, the WP1 screening threshold's path and hash, generator commit), the metric definitions, both views, the alignment rule, the findings and the figure caption |
...
The command selects **every recording whose decoded fingerprint puts it at a pitch of this ladder** (never
a filename list and never a folder), orders the ladder by decoded pitch, re-checks all 14 source SHA-256
values against the bytes together with the decoded settings and grid of each recording, reads the decision
threshold from the committed WP1 provenance and
...
- **sole-pair observed-discrepancy screening threshold** — the committed WP1 value, 19.37008103465545 mm/s
...
- **No measured level differs from any other by more than the screening threshold.** Across all 78 unordered pairs
  the largest absolute per-knot mean-profile difference is **17.37 mm/s** (`res/1-2.BDD` vs
  `res/1-6.BDD` at 43.83 mm), **0.897** of the 19.37 mm/s screening threshold, and **not one of the 78 pairs puts a
...
The corrected ladder is a setting-based selection of **13 decoded cycle counts from 14 recordings** over the
same window on the same 1.85 mm gate grid — 2 (`burst_len/2.BDD`) to 32 (`burst_len/32.BDD`) — and it carries
the plan's second question: *where is the empirical transition, and can 18 be separated from 20 cycles?*
Selection is by the decoded fingerprint, not by folder: the reference level (10 cycles) is not a file of
`burst_len/` at all but the shared reference condition, realized by both reference recordings
(`prf/600.BDD`, `res/1-8.BDD`), so 12 of the 13 levels are a single recording and the shared reference level
carries two named realizations (`findings.realizations`). **One duplicated setting is not replicated coverage
of the axis**, and each level's numbers are the unweighted mean of its realizations' numbers. The artefacts
are:

| Path | What it is |
|---|---|
| `burst-levels.csv` | 13 rows, one per decoded cycle count (selected from 14 recordings, with the shared reference level carrying two named realizations): common-duration mean, robust spread (IQR), RMS, zero fraction (dropout), native-grid gradient and correlation length, and the matched full-record temporal metrics (ACF e-folding lag, in-band power share above 10 Hz, spectral centroid, RMS bandwidth) |
| `burst-pairs.csv` | 78 rows, every unordered pair of the 13 levels: the signed `short - long` per-gate mean difference at the shared knots, the knots that clear the WP1 screening threshold and the depth ranges where they do, and the short-to-long change of the dropout, variance, smoothing and bandwidth metrics |
| `burst-ladder.provenance.json` | the binding (manifest hash, all 14 source hashes, the WP1 screening threshold's path and hash, generator commit), the metric definitions, both views, the observed same-settings temporal discrepancy, the findings and the figure caption |
...
The command selects **every recording whose decoded fingerprint puts it at a burst length of this ladder**
(never a filename list and never a folder), orders the ladder by decoded cycle count, re-checks all 14
source SHA-256 values against the bytes together with
...
- **common physical support** — the intersection of the 14 decoded depth ranges,
  10.1626666667-100.812666667 mm. All 14 recordings carry the identical 50-gate 1.85 mm grid, so this
...
- **matched temporal view** — every file's full record in non-overlapping segments of the same 86
  profiles on one shared profile period (0.02238922387935217 s, the mean of the 14 implied periods,
  which agree to 1.7e-5 relative): frequency resolution 0.5194 Hz at a 22.33 Hz Nyquist limit, 5-7
...
- **18 cycles cannot be separated from 20.** `burst_len/18.BDD` vs `burst_len/20.BDD`: max `|diff|`
  **11.7519 mm/s** at 91.563 mm = **0.607** of the 19.37 mm/s screening threshold, **0 of 50** knots above it.
  The whole 16-20-cycle region is below the screening threshold: 16 vs 18 = **0.650**, 16 vs 20 = **0.256**, and all
  three unordered pairs have **zero** knots above the screening threshold. The verdict recorded in the provenance
  is therefore that this evidence cannot choose between 18 and 20; what remains unidentifiable is any
  burst effect smaller than the screening threshold, and the pitch x burst interaction (the two ladders meet only
  at the reference).
- **Most of the ladder is below the screening threshold, but it is not empty.** Across all 78 pairs the largest
  absolute per-knot mean-profile difference is **44.0313 mm/s** (`burst_len/4.BDD` vs
  `burst_len/28.BDD`, at the first gate, 10.163 mm) = **2.273** of the screening threshold. **22 of 78** pairs put
  at least one knot above the screening threshold, always at a handful of gates (never more than 5) and never
  across the support; **17** of those 22 involve the two longest bursts (28, 32).
- **The dropout and variance "knees" are one recording, not a trend.** No metric falls monotonically
  ...
  recording, or drift cannot be separated from a single recording at that level, with the one duplicated
  reference level sitting elsewhere in the ladder.
- **The longer pulse is not shown to smooth the profile or to narrow the band.** The native correlation
  ...
  **0.0885-0.1387** (spread 0.0502), the spectral centroid **3.504-4.434 Hz** and the RMS bandwidth
  **4.044-4.725 Hz**, while the two committed same-settings recordings differ by **0.02791** in the same
  share, **0.776 Hz** in the centroid and **0.376 Hz** in the bandwidth — the ladder's spread is only
  ~1.8x that observed discrepancy. The direction is the one a longer pulse predicts (the
  ...
  The ACF e-folding lag takes three values (0.08956 s at 11 levels, 0.10075 s at the grouped reference
  level, 0.11195 s at `burst_len/32.BDD`), and the reference pair's own lag difference is one profile
  period (0.02239 s), so a one-lag change is smaller than that observed discrepancy by construction.
...
The corrected ladder is a setting-based selection of **5 decoded pulse-repetition periods from 6
recordings** over the same window on the same 1.85 mm gate grid — 400 µs (`prf/400.BDD`) to 800 µs
(`prf/800.BDD`) — and it carries the plan's third question: *does the committed 400-µs setting have
inadequate velocity or temporal-bandwidth headroom, and is a 250-µs acquisition therefore justified?*
Selection is by the decoded fingerprint, not by folder: the 600 µs level is realized by both reference
recordings (`prf/600.BDD`, `res/1-8.BDD`), so 4 of the 5 levels are a single recording and the shared
reference level carries two named realizations (`findings.realizations`). **One duplicated setting is not
replicated coverage of the axis**, and each level's numbers are the unweighted mean of its realizations'
numbers. The artefacts are:

| Path | What it is |
|---|---|
| `prf-levels.csv` | 5 rows, one per decoded period (selected from 6 recordings, with the 600 µs reference level the unweighted mean of two named realizations): common-duration mean, robust spread, RMS, zero fraction, native-grid gradient and correlation length, the actual profile rate from the timestamps, the `|v| / Vmax` load with its warning fractions, the wrap-like discontinuity count and step scale, and the matched-segment temporal metrics (segment length, resolution, usable bandwidth, ACF e-folding lag, share of in-band power below the 8.33 Hz marker) |
| `prf-pairs.csv` | 10 rows, every unordered pair of the 5 levels: the signed `faster - slower` per-gate mean difference at the shared knots, the knots that clear the WP1 screening threshold and the depth ranges where they do, the load/wrap/zero-fraction change, and the band-mean spectral level difference on the shared comparison bands |
| `prf-ladder.provenance.json` | the binding (manifest hash, all 6 source hashes, the WP1 screening threshold's path and hash, generator commit), the audit of the key's scaled velocity range, the metric definitions, both views, the decision findings and the figure caption |
...
The command selects **every recording whose decoded fingerprint puts it at a period of this ladder** (never
a filename list and never a folder), orders the ladder by decoded PRF period, re-checks all six source
SHA-256 values against the bytes together with
...
- **common-duration view** — the largest integer number of nominal 500-RPM revolutions (0.12 s each)
  ...
  timestamps (537/431/360/360/309/270 profiles), because the profile rate differs across the ladder and
  the profile *count* therefore cannot be the common quantity.
- **common physical support** — the intersection of the six decoded depth ranges,
...
- **matched temporal view** — the files do **not** share a profile rate (66.72 Hz at 400 µs down to
  ...
  inside it — 106/85/71/71/61/53 profiles, spans 1.5737-1.5490 s (1.6 % apart), frequency resolutions
...
- **observed same-settings temporal discrepancy** — the same-settings WP1 pair (`prf/600.BDD` vs `res/1-8.BDD`) through
  ...
  0.02099. A smaller cross-level spectral difference does not demonstrate an axis effect relative to that observation.
...
- **The 400-µs setting has the *most* velocity headroom of the five levels, not the least.** In its
  ...
  grows as the period lengthens: **1.0586** at 600 µs (the unweighted mean of that level's two named
  realizations, which carry 0.5 samples beyond the limit and 1 wrap-like step between them),
  **1.0625** at 500 µs, **1.2109** at 800 µs, where the recording also carries 1 sample beyond the
  limit, 3 wrap-like steps and a largest step of
...
- **The 400-µs setting also has the widest usable temporal bandwidth of the five levels.** Its profile rate
  ...
  with the 8.333 Hz marker inside it (0.8299-0.8794 of each level's in-band power lies below the
  marker). The ACF e-folding lag takes only 0.0783-0.1008 s across the ladder — one lag step is
...
- **A 250-µs acquisition is not justified by this evidence.** Both counts the plan names are
  ...
  PRF-axis recordings is 196.53 mm/s (`prf/500.BDD`), still 0.85 of the 400-µs limit. Unidentifiable here: a
...
- **Six of the ten pairs clear the WP1 screening threshold somewhere, and the clearances are local.** The
  largest absolute per-knot mean-profile difference is **38.14 mm/s** (`prf/400.BDD` vs
  `prf/600.BDD` at 13.8627 mm) = **1.969** of the 19.37 mm/s screening threshold, over 3 knots in two runs
  (10.1627, 13.8627-15.7127 mm). The other clearances sit at 1-7 knots: 400 vs 500 (7 knots in three
  runs, 10.1627, 13.8627-15.7127 and 34.2127-39.7627 mm), 400 vs 700 (1 knot), 400 vs 800 (2 knots),
  500 vs 800 (5 knots, out to 82.31 mm) and 600 vs 800 (5 knots, out to 69.36 mm). Only four of the ten
  pairs — `prf/500.BDD` vs `prf/600.BDD`, `prf/500.BDD` vs `prf/700.BDD`, `prf/600.BDD` vs `prf/700.BDD`
  and `prf/700.BDD` vs `prf/800.BDD` — are below the screening threshold at every
...
- **A spectral axis effect is not demonstrated relative to the sole-pair discrepancy.** Band-mean spectral level
  differences between levels run 1.23-3.40 dB in magnitude, while the same-settings WP1 pair differs
  by 3.532 dB in the same resummarisation; the direction (the shorter period's band level below the
  longer's in all 10 pairs, on the band median) is not larger than that observed discrepancy, and with 5-10 segments per level
...
rate and usable bandwidth, `prf-pairs.csv` for the screening threshold comparison and the band level
...
filename list: `tgc` (9 levels from 10 recordings, `tgc/0.BDD` … `tgc/40.BDD` plus the base state at
19.9216 dB, keyed by the decoded TGC start in dB) and `em_pow` (3 levels from 4 recordings,
`em_pow/low.BDD`, `em_pow/high.BDD` and the base state `medium`, keyed by the instrument's own
...
screening threshold of **19.37 mm/s**.
...
- **TGC is screened through the committed representation, not a gain ladder.** Op word 23 stays `0`
  ...
  of the TGC axis and all 4 of the power axis (9 distinct word-24 values on the TGC axis, one shared
  value on the power axis).
...
contains the plan's 10.163-96.743 mm window. Both axes carry the base state as one of their own levels
rather than bracketing it from outside: the other manifest rows carry TGC start 19.92 dB / emitting power
`medium`, and **the base state is itself a level of both axes** — the reference recordings realize TGC
start 19.9216 dB and emitting power `medium`
— so each axis's focus pair is that level against the next setting above it: `prf/600.BDD` vs
`tgc/25.BDD` on the TGC axis and `prf/600.BDD` vs `em_pow/high.BDD` on the power axis
(`findings.axes.tgc.base_state`, `findings.axes.em_pow.base_state`).

The whole screen is therefore **12 levels over the two axes, selected from 14 recordings and compared as
39 pairs**; **2 levels are flagged** and **17 of the 39 pairs clear the screening threshold somewhere**
(`findings.screen_summary`).

What the velocity-only screen found:

- **Two of the twelve screened levels are flagged, both on the TGC axis.** `tgc/0.BDD` is dropout-limited:
  ...
  **0.8111** blank at one gate. `tgc/40.BDD` is dropout- **and** spread-limited: 0.0388 overall (1
  ...
  73.06 mm — **4.97×** the axis's median per-gate spread. Both are single-recording facts: every other
  level of the axis is one recording, the base state carries the two reference realizations, there is no
  acquisition order, and nothing here tells the gain from the flow, from drift or from one bad recording.
- **The power axis is unremarkable in velocity.** Its whole ladder — `prf/600.BDD` (the base state
  `medium`) vs `em_pow/high.BDD` — differs by at most **13.39 mm/s** at 58.26 mm = **0.691** of the
  screening threshold, with **0** of 50 knots above it; no level carries a majority-blank gate or a
  disproportionate per-gate spread (0 of 3 flagged).
- **17 of the 36 TGC pairs clear the screening threshold somewhere, locally.** The largest absolute per-knot
  mean-profile difference in the screen is **47.10 mm/s** (`tgc/30.BDD` vs `tgc/40.BDD` at 73.06 mm)
  = **2.432** of the screening threshold, over 2 knots in one run (73.06-74.91 mm). Most clearances are 1-3
  knots and none spans the support, so this is a localised mid/near-depth signal against a
  mostly-single-recording, orderless ladder — not a global velocity bias.
- **The base state's own focus pair is below the screening threshold.** `prf/600.BDD` vs `tgc/25.BDD`
  (carrying 19.9216 dB and 24.9412 dB) differ by at most **15.4 mm/s**
  at 63.81 mm = **0.795** of the screening threshold over 0 of 50 knots. The `em_pow` focus pair is the whole
  power ladder, at **0.691** of the screening threshold.
...
no wider ladder first — because two of the twelve screened levels already fail the dropout/spread screen for reasons
...
every within-axis pair against the screening threshold and the depth ranges where it clears,
