...
of the two alternatives plan §8.2 R3 allows, with E128 an ordinary sparse point rather than a gated one. **No
E20-to-E64 displacement is called a plateau test**: one recording per level against a single reference cannot
decide whether the axis has levelled off, and the corrected evidence shows no committed variation of this axis
at all, so the design does not attempt one. Every condition in the set is unconditional.
