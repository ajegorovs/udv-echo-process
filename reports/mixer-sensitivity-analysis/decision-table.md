...
|---|---|---|---|---|---|---|
| **R1 — 0.247 mm x 365 gates** (`res/0-2.BDD`) | Measured. Against 0.617 mm on the coarser level's own 141 knots: mean abs diff **1.974 mm/s**, max **6.1136 mm/s** at 44.696 mm = **0.3156** of the screening threshold, **0 of 141** knots above it (`resolution-pairs.csv`, row `res/0-2.BDD - res/0-6.BDD`; `resolution-ladder.provenance.json` `findings.information`). The finer pitch's extra gates carry **0.0722 %** of the spatial variance (sub-knot detail RMS 0.5814 mm/s, peak 2.7382 mm/s = 0.141 of the screening threshold). Its own native correlation length is 10.1133 mm = 41 pitches (`resolution-levels.csv`). | **No.** 0 of 141 knots above it; 0.3156 x screening threshold. The largest of all 78 pairs is **0.8966** (1-2 vs 1-6 at 43.8327 mm, 0 knots above), so no level differs from any other by more than repeat-plus-drift anywhere in the common support. | None demonstrated for finer pitch. This is the plan's named question and the measured answer is "not demonstrated": the level difference and the structure the extra gates add both sit below the screening threshold. | None — not acquired. | `replace` (measured; dropped) — the fine end of the first augmentation is **0.617 mm**. The instrument accepts the 0.247 mm rung; it is dropped for information, not for feasibility. | Two recordings per pitch whose per-knot difference clears the screening threshold, or a replicated sub-knot detail above the screening threshold (the detail is measured inside the finer record alone, so replication is the only thing that separates it from noise). |
...
| **R4 — 2.960 mm x 31 gates** (`res/3-0.BDD`) | Measured. The coarsest pitch in the set: 30 supported gates, native correlation length **11.84 mm = 4.0 pitches**, and its largest per-knot difference to any of the other 12 levels is **10.4324 mm/s = 0.5386** of the screening threshold (`resolution-ladder.provenance.json` `findings.coarsest_pitch`). | **No.** 0.5386 x screening threshold against a coarser grid is impossible; nothing finer or coarser was measured. | Retained as the **coarse pitch corner** of the crossing (CC3/CC4): it still samples the structure 4 times per correlation length, so it is the coarse end the evidence supports rather than merely tolerates. | Same as R2: resolution + gates writers exist; the burst field is the gap. | `keep` (measured at the reference burst; acquire only *inside* the crossing) | A replicated collapse of the correlation length at 2.960 mm (below the ~4 samples per correlation length the level measures today), or a difference to a finer level that clears the screening threshold at more than a knot in a replicated design. |
