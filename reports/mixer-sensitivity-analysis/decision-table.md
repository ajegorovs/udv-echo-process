...
- **The repeatability bound** is the committed WP1 envelope, **19.37008103465545 mm/s**
  (`max_gate_abs_mean_difference_mm_s`, gate 39, 82.3127 mm) from `reference-repeat.provenance.json`. It bounds
...
- **No cell is causal.** Where a pair clears the envelope, that is a fact about two single recordings — not
...
- `Exceeds repeatability?` is `N/A` exactly where the committed set contains no measured variation of that axis.
...
| `reference-repeat.provenance.json` | `a58a83a28b667fd1998ac81f2a3553925ce2951fff4fdb2431d3988e0924d263` | the envelope, the metric definitions, both time views, the temporal floor curves |
...
| `resolution-pairs.csv` | `1278c42f7c66eb93d7224b543d6c70814dd4a000aaf70c8c76ab9774781e703f` | all 78 pairs: per-knot difference, knots above the envelope, depth ranges, sub-knot detail |
| `resolution-ladder.provenance.json` | `e5e08dd71f666ec014521cd6a82efa905e4691d91580e72d4fe67c45dae75793` | `findings.envelope_gate`, `findings.information`, `findings.coarsest_pitch`, `findings.limitations` |
...
| `prf-pairs.csv` | `189a69477acd9970a49ff0ca4e6e87a07566cfd0bc3f24f5ea30c83c1c09ea56` | all 10 pairs: per-knot difference, knots above the envelope, band-level differences |
...
| `gain-power-pairs.csv` | `cf231d6290d5b6ea67dffec907d9eff77e47a91ce0b6c7408c573e6f8d1f455f` | all 29 within-axis pairs against the envelope |
...
**Exceeds repeatability? (where/depth or N/A)** · **Information gained** · **Automation needed** · **Verdict** ·
...
| Candidate | Existing evidence | Exceeds repeatability? (where/depth or N/A) | Information gained | Automation needed | Verdict | Measurement that overturns it |
|---|---|---|---|---|---|---|
| **R1 — 0.247 mm x 365 gates** (`res/0-2.BDD`) | Measured. Against 0.617 mm on the coarser level's own 141 knots: mean abs diff **1.974 mm/s**, max **6.1136 mm/s** at 44.696 mm = **0.3156** of the envelope, **0 of 141** knots above it (`resolution-pairs.csv`, row `res/0-2.BDD - res/0-6.BDD`; `resolution-ladder.provenance.json` `findings.information`). The finer pitch's extra gates carry **0.0722 %** of the spatial variance (sub-knot detail RMS 0.5814 mm/s, peak 2.7382 mm/s = 0.141 of the envelope). Its own native correlation length is 10.1133 mm = 41 pitches (`resolution-levels.csv`). | **No.** 0 of 141 knots above it; 0.3156 x envelope. The largest of all 78 pairs is **0.8966** (1-2 vs 1-6 at 43.8327 mm, 0 knots above), so no level differs from any other by more than repeat-plus-drift anywhere in the common support. | None demonstrated for finer pitch. This is the plan's named question and the measured answer is "not demonstrated": the level difference and the structure the extra gates add both sit inside the bound. | None — not acquired. | `replace` (measured; dropped) — the fine end of the first augmentation is **0.617 mm**. The instrument accepts the 0.247 mm rung; it is dropped for information, not for feasibility. | Two recordings per pitch whose per-knot difference clears the envelope, or a replicated sub-knot detail above the bound (the detail is measured inside the finer record alone, so replication is the only thing that separates it from noise). |
| **R2 — 0.617 mm x 145 gates** (`res/0-6.BDD`) | Measured. Member of the 78 pairs, none of which clears the envelope; its own mean 25.6882 mm/s, robust spread 31.4933 mm/s, zero fraction 0.009661, native correlation length 9.8667 mm = 16 pitches (`resolution-levels.csv`). | **No.** No pair of the 78 puts a knot above the envelope. | Retained as the **fine pitch corner** of the pitch x burst crossing (§5, CC1/CC2): the interaction is not estimable from the committed set at all (`resolution-ladder.provenance.json` `findings.limitations`: the two ladders meet only at the reference). | Per-point resolution + gates writers already exist (`actuator.PARAMETER_WRITE_ORDER` is `(RESOLUTION, GATES)`); the crossing cells additionally need the burst field — see §5. | `keep` (measured at the reference burst; acquire only *inside* the crossing) | A replicated pitch effect at 0.617 mm against a coarser level, or a replicated loss of structure by 0.617 mm — either would move this corner. |
| **R3 — 1.233 mm x 74 gates** (`res/1-2.BDD`) | Measured. Participant in the largest pair of the ladder: 1-2 vs 1-6 = **17.368 mm/s** at 43.8327 mm = **0.8966** of the envelope, **0 knots above** (`resolution-ladder.provenance.json` `findings.envelope_gate`); own correlation length 12.3333 mm = 10 pitches. | **No.** | None that the two retained corners do not already carry: an intermediate rung sits between two pitches that are indistinguishable on this evidence. | None — not acquired. | `defer` (measured; not reacquired) — revisit only against a named scale threshold. | A replicated pitch effect localized between 0.617 and 2.960 mm (structure on a scale between the two corners) — that would put a rung back inside them. |
| **REF — 1.850 mm x 50 gates** (`res/1-8.BDD`) | Measured. One of the two members of the only same-settings pair (`prf/600.BDD` vs `res/1-8.BDD`, 669 / 517 profiles, 14.956 / 11.5529 s), so it is the comparand the envelope is computed against. | **N/A** — it is the envelope's own comparand. | The anchor of every new condition and the **reference control** that bounds drift inside the new run (§5, REF-CTRL). | None: the reference writes nothing but resolution and gates. | `keep` (measured; the controls are acquired) | A control set whose per-gate spread exceeds the committed envelope would say the bound is not transferable to the new run — see §5. |
| **R4 — 2.960 mm x 31 gates** (`res/3-0.BDD`) | Measured. The coarsest pitch in the set: 30 supported gates, native correlation length **11.84 mm = 4.0 pitches**, and its largest per-knot difference to any of the other 12 levels is **10.4324 mm/s = 0.5386** of the envelope (`resolution-ladder.provenance.json` `findings.coarsest_pitch`). | **No.** 0.5386 x envelope against a coarser grid is impossible; nothing finer or coarser was measured. | Retained as the **coarse pitch corner** of the crossing (CC3/CC4): it still samples the structure 4 times per correlation length, so it is the coarse end the evidence supports rather than merely tolerates. | Same as R2: resolution + gates writers exist; the burst field is the gap. | `keep` (measured at the reference burst; acquire only *inside* the crossing) | A replicated collapse of the correlation length at 2.960 mm (below the ~4 samples per correlation length the level measures today), or a difference to a finer level that clears the envelope at more than a knot in a replicated design. |
...
| **400 µs** (`prf/400.BDD`) | Measured. 545 profiles, 8.1535 s, profile rate 66.7198 Hz, margin fractions 0.000335 at half-scale / 0 / 0 / 0 at the limit, 0 samples beyond, 0 wrap-like steps. | **Yes** — as a pairwise effect only: 400 vs 500 puts **7** knots above the envelope in three runs (10.1627; 13.8627-15.7127; 34.2127-39.7627 mm), max **37.5148 mm/s** at 13.8627 mm = **1.9367** x envelope; 400 vs 600 clears at 2 knots, 400 vs 700 at 1. For attribution see the PRF limits below. | The known low-period anchor: the setting the plan's 250 µs question is decided against, and the only level that keeps margin at both the velocity limit and the bandwidth ceiling. | None — already measured; the first augmentation adds nothing at PRF. | `keep` (measured; no acquisition) | A replicated clearance that survives control-based drift bounding, or a different setpoint at which this level's own load approaches its limit. |
| **600 µs** (`prf/600.BDD`) | Measured. The reference recording: 669 profiles, 14.956 s, load 0.9219, band 22.0176 Hz, rate 44.6643 Hz; member of the WP1 pair. | **Yes** — as a pairwise effect only: 600 vs 800 puts **8** knots above the envelope in three runs (19.4127-21.2627; 54.5627-56.4127; 65.6627-71.2127 mm), max 22.4215 mm/s = 1.1575 x envelope; 600 vs 700 at 2 knots (80.4627-82.3127 mm). | It is the reference condition itself: the anchor every new condition is defined around, and the period the whole augmentation holds fixed. | None: `prf_us` is run-wide and the augmentation never moves it. | `keep` (measured; the reference) | A replicated PRF effect that separates the 400/600 pair from drift, or a recorded wrap-like event at this period (measured today: 0). |
| **800 µs** (`prf/800.BDD`) | Measured. 539 profiles, 16.0267 s, load **1.2109**, **1** sample beyond the limit, **3** wrap-like steps (largest step 1.4531 `Vmax`), narrowest usable bandwidth 16.4678 Hz, rate 33.5690 Hz. | **Yes** — as a pairwise effect only: 800 vs 600 clears at 8 knots; 800 vs 400 at 2; 800 vs 500 at 5; 800 vs 700 clears nowhere (0.9965 x envelope, the closest non-clearance in the ladder). | None sought: the ladder's own load ordering says the long-period side is where the velocity scale is under pressure, and this is the level that shows it most. | None — not acquired. | `defer` (measured; not reacquired) | A stated question needing a longer period (e.g. a deeper window at a different setpoint), and then only with the echo/energy diagnostic in place — this is the one level whose measured load exceeds 1.0. |

**What the PRF clearances are not.** Eight of the ten pairs clear the envelope; every clearance is a handful of
...
| **64** (new) | Not measured, as above. | **N/A** | Moderate averaging: the level that decides whether the axis has plateaued, and the gate for 128. | As for 8 (one job today, or a per-point writer for one run). | `keep` (acquire) | A control spread above the envelope inside the emissions jobs; or the axis proving unreadable (see 8). |
...
| Candidate | Existing evidence | Exceeds repeatability? (where/depth or N/A) | Information gained | Automation needed | Verdict | Measurement that overturns it |
|---|---|---|---|---|---|---|
| **4 cycles** (`burst_len/4.BDD`) | Measured. 50 gates, 1.85 mm, ref velocity mean 24.0755 mm/s, robust spread **34.3782 mm/s** (the ladder's largest one-step change, **+12.88**, and a *rise*), zero fraction 0.007692, native correlation length 12.95 mm = 7 pitches (`burst-levels.csv`; `findings.knees.robust_spread_mm_s`). | **Yes**, as a pairwise fact: 4 vs 28 puts knots above the envelope (max **44.0313 mm/s** at 10.1627 mm = **2.2732** x envelope; **21 of 66** pairs clear somewhere and **16** of those involve the two longest bursts). No pair in the 16-20-cycle region clears. | The short-burst corner of the crossing (CC1/CC3): half of the only design in which the pitch x burst interaction can be estimated at all. | `burst_length` is dialog-only and run-wide today (`actuator.DIALOG_ONLY_PARAMETERS`, `DialogField.BURST_LENGTH`; `campaign` carries it as a shared value). With today's writers the crossing runs as one job per burst length; a per-point burst write is needed for one randomized run. | `keep` (measured at the reference pitch; acquire only the crossing cells) | Two recordings at 4 cycles showing the spread rise is a property of the level rather than of one recording — or a replicated interaction contrast that makes the crossing's own cells redundant. |
| **10 cycles** (reference) | Measured — the reference condition (`res/1-8.BDD`, `prf/600.BDD`, burst 10, 1.85 mm). | **N/A** — it is the comparand of the envelope. | The anchor: every new condition and every reference control is defined by not moving it. | None. | `keep` (measured; the controls are acquired) | A control spread above the envelope inside the new run. |
| **20 cycles** (`burst_len/20.BDD`) | Measured. The plan's focus pair against 18 cycles: max abs diff **11.7519 mm/s** at 91.5627 mm = **0.6067** of the envelope, **0 of 50** knots above; the whole 16-20 region is inside the bound (16 vs 18 = 0.6504, 16 vs 20 = 0.2560, 3 pairs, 0 clearing) (`burst-pairs.csv`; `findings.focus_18_vs_20`, `findings.focus_window_16_20`). | **No** against 18, and **No** anywhere in the 16-20 region. Against other levels it clears only in the long-burst pairs recorded on the 4-cycles and 32-cycles rows. | Nothing distinguishable from 18 cycles; its one step from 18 changes only within the bound (correlation length +1.85 mm, median gradient -0.3215 mm/s/mm, robust spread +2.0610 mm/s, zero fraction -0.00081, >10 Hz share -0.00277, ACF lag 0). | None — not acquired. | `replace` (measured; dropped in favour of **18 cycles**) — chosen on cost, **not** on any claimed effect: see below. | Two recordings per cycle count in the 16-20 region separating the level effect from drift; that would let the pair be decided on evidence instead of on cost. |
...
| Candidate | Existing evidence | Exceeds repeatability? (where/depth or N/A) | Information gained | Automation needed | Verdict | Measurement that overturns it |
|---|---|---|---|---|---|---|
| **medium** (reference) | Measured — every committed file; it is the base state's own value. | **N/A** | The comparand for the one higher condition; nothing else. | None. | `keep` (measured; the reference) | A control spread above the envelope inside the diagnostic run. |
...
| **REF-CTRL — the reference condition, beginning/middle/end of each run** (new repeats) | The only committed same-settings repeat is the WP1 pair — **one** repeat, which is why every verdict above is bounded by a drift-inclusive envelope rather than a repeatability estimate (`reference-repeat.provenance.json` `envelope.scope`). | **N/A** — this row *is* the new measurement of the bound. | Three same-settings recordings per run in place of one committed repeat: a within-run drift bound with two independent differences instead of one, and the direct check that the committed 19.3701 mm/s bound transfers to the new run. | None: the reference writes only resolution and gates, and the campaign runs the definition's point order literally, so the controls are a definition-authoring rule (position), not a code change. | `keep` (acquire, 3 per run) | A control set whose per-gate spread exceeds the envelope: that would say the new run is drifting more than the committed dataset and the control count/duration must be raised before any other verdict is read. |
...
### 8.3 Reference controls

Three independent recordings of the **reference condition** (1.850 mm x 50 gates, burst 10, emissions 20,
...
- **Fixed duration.** The committed durations span **8.1535-16.0267 s** and the comparison windows used by the
  ...
  `views.temporal`) — or a control spread above the envelope.
- **Reference replication count.** One committed repeat cannot say how many repeats are enough. Smallest
  ...
  two independent within-run differences. Revised by: a control spread whose per-gate mean difference exceeds
...
1. `resolution-pairs.csv` — the `res/0-2.BDD` - `res/0-6.BDD` row (`max_abs_difference_over_envelope` 0.3156,
   `knots_above_envelope` 0) and the `res/1-2.BDD` - `res/1-6.BDD` row (0.8966), then
...
**Limitations carried by every verdict here.** One same-settings repeat only, so the bound is repeatability
