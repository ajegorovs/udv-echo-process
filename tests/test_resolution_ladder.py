...
# ── slice 9: the residual R2 semantics of the two resolution verdicts ──────────────────────
#: Plan §9.1's third finding, §9.4 step 2 and §9.5: one *observed* same-settings discrepancy is a
#: screening reference, so the resolution verdicts may not turn it into an inference about a
#: smaller difference — the live positive phrases are the two §9.4 step 2 names, quoted here as
#: they stand in the verdicts, with the two names §9.5 pairs them with.
DETECTION_LIMIT_PHRASES: tuple[str, ...] = (
    "would be invisible",
    "cannot resolve",
    "indistinguishable",
    "unresolvable",
...
#: The wording the correction puts in place of the retired inference (plan §9.1: *replace only the
#: inference* — the current single-repeat evidence does not support distinguishing the difference
#: from the observed same-settings discrepancy), and the role it must name instead.
SINGLE_REPEAT_LIMIT_CLAUSE = (
    "the current single-repeat evidence does not support distinguishing the difference from the "
    "observed same-settings discrepancy"
)
SCREENING_REFERENCE_ROLE = "screening reference"

#: The resolution-owned artefacts this slice regenerates, scanned as bytes.
RESOLUTION_ARTEFACT_NAMES = (
...
def _detection_limit_phrases_in(text: str) -> list[str]:
    """Every retired detection-limit phrase this text still carries, in the declared order."""
    lowered = text.lower()
    return [phrase for phrase in DETECTION_LIMIT_PHRASES if phrase in lowered]


def _verdict_texts(model) -> dict[str, str]:
    """The two verdict sentences, keyed by the module constant that writes them.

    Both are published inside the focus and coarsest findings statements, so the constants and the
    emitted documents have to say the same thing.
    """
    from udv_echo_process.analysis import resolution_ladder as module

    del model  # the verdicts are written from the numbers; the constants are the live wording
    return {
        "_FOCUS_VERDICT": module._FOCUS_VERDICT,
        "_COARSEST_VERDICT": module._COARSEST_VERDICT,
    }


def test_the_two_resolution_verdicts_state_no_detection_limit(ladder) -> None:
    """R2: neither live verdict may infer a detection limit from one observed discrepancy."""
    for where, text in _verdict_texts(ladder).items():
        assert _detection_limit_phrases_in(text) == [], where


def test_every_reviewer_visible_resolution_string_states_no_detection_limit(ladder) -> None:
    """The same rule over the whole emitted surface, the module's own source prose included."""
    for where, text in _r7_r8_texts(ladder).items():
        assert _detection_limit_phrases_in(text) == [], where


def test_the_committed_resolution_artefacts_state_no_detection_limit() -> None:
    """The generated bytes, not only the source: this slice owns their regeneration."""
    for name in RESOLUTION_ARTEFACT_NAMES:
...
def test_each_verdict_replaces_the_limit_claim_with_the_screening_reference(ladder) -> None:
    ...
    assert SINGLE_REPEAT_LIMIT_CLAUSE in texts["_FOCUS_VERDICT"]
    for where, text in texts.items():
        assert SCREENING_REFERENCE_ROLE in text.lower(), where


def test_the_detection_limit_check_has_teeth(ladder) -> None:
    """The retired phrases are caught when they are spliced back into a verdict."""
    texts = _verdict_texts(ladder)
    for where, text in texts.items():
        assert _detection_limit_phrases_in(text) == [], where
    assert _detection_limit_phrases_in(
        texts["_COARSEST_VERDICT"] + " A smaller difference would be invisible here."
    ) == ["would be invisible"]
    assert _detection_limit_phrases_in(
        texts["_FOCUS_VERDICT"] + " One same-settings repeat cannot resolve it."
    ) == ["cannot resolve"]
